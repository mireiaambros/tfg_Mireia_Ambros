...
def process_video(path, model):
    ...
    try :
      while True:
          ...
          if ret and current_frame < frame_length:
              ...
              preds = post_act(preds.cpu()).detach().numpy() #Tampoc entenc aquest pas! MIRAR
    ...
    # Sums the probabilities of each image in the video
    preds = sum(prob_list)
...
def main():
        ...
        if os.path.isdir(os.path.join(root, folder)):
            for file in sorted(os.listdir(root + folder)):
                if file.find("mp4")!=-1:
                    ...
                    prediction = process_video(video_path, model)
    ...
    print(folder2)
    print(y_pred)
    df.to_csv('pred_df_all_videos.csv', index=None, columns=None)
...
def pretty_plot_confusion_matrix(df_cm, annot=True, cmap="Oranges", fmt='.2f', fz=15,
    ...
    plt.show()
    plt.savefig('my_plot_all_videos.png')
